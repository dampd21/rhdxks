  ...
  async fetch(request, env) {
    ...
    if (db && !globalThis.__SHERPA_AUTO_MIGRATED__) {
      try {
        ...
        globalThis.__SHERPA_AUTO_MIGRATED__ = true;
      } catch (e) { }
    }
    var url = new URL(request.url);
    ...
    if (pathname === '/api/community/posts' && request.method === 'GET') {
      try {
        return await COMMUNITY_V2_handlePostList(request, env);
      } catch (e) {
        console.error('COMMUNITY_V2_handlePostList error:', e);
        return jsonResp({ ok: false, error: e.message || 'community post list failed' }, 500);
      }
    }

    if (pathname === '/api/community/posts' && request.method === 'POST') {
      try {
        return await COMMUNITY_V2_handlePostCreate(request, env);
      } catch (e) {
        console.error('COMMUNITY_V2_handlePostCreate error:', e);
        return jsonResp({ ok: false, error: e.message || 'community post create failed' }, 500);
      }
    }

    if (/^\/api\/community\/posts\/\d+$/.test(pathname) && request.method === 'GET') {
      try {
        return await COMMUNITY_V2_handlePostDetail(request, env, pathname);
      } catch (e) {
        console.error('COMMUNITY_V2_handlePostDetail error:', e);
        return jsonResp({ ok: false, error: e.message || 'community post detail failed' }, 500);
      }
    }

    if (/^\/api\/community\/posts\/\d+\/comments$/.test(pathname) && request.method === 'GET') {
      try {
        return await COMMUNITY_V2_handleCommentList(request, env, pathname);
      } catch (e) {
        console.error('COMMUNITY_V2_handleCommentList error:', e);
        return jsonResp({ ok: false, error: e.message || 'community comment list failed' }, 500);
      }
    }

    if (/^\/api\/community\/posts\/\d+\/comments$/.test(pathname) && request.method === 'POST') {
      try {
        return await COMMUNITY_V2_handleCommentCreate(request, env, pathname);
      } catch (e) {
        console.error('COMMUNITY_V2_handleCommentCreate error:', e);
        return jsonResp({ ok: false, error: e.message || 'community comment create failed' }, 500);
      }
    }

    if (pathname === '/api/community/attendance/status' && request.method === 'GET') {
      try {
        return await COMMUNITY_V2_handleAttendanceStatus(request, env);
      } catch (e) {
        console.error('COMMUNITY_V2_handleAttendanceStatus error:', e);
        return jsonResp({ ok: false, error: e.message || 'community attendance status failed' }, 500);
      }
    }

    if (pathname === '/api/community/attendance/feed' && request.method === 'GET') {
      try {
        return await COMMUNITY_V2_handleAttendanceFeed(request, env);
      } catch (e) {
        console.error('COMMUNITY_V2_handleAttendanceFeed error:', e);
        return jsonResp({ ok: false, error: e.message || 'community attendance feed failed' }, 500);
      }
    }

    if (pathname === '/api/community/attendance/checkin' && request.method === 'POST') {
      try {
        return await COMMUNITY_V2_handleAttendanceCheckin(request, env);
      } catch (e) {
        console.error('COMMUNITY_V2_handleAttendanceCheckin error:', e);
        return jsonResp({ ok: false, error: e.message || 'community attendance checkin failed' }, 500);
      }
    }
